(function () {
  ...
  function potapAdvice(key) {
    ...
    const scenarios = {
      traffic: {
        recommendation: 'Я бы собрал первый тест из четырёх частей: одна аудитория, одно предложение, один канал и один измеримый результат. Этого достаточно, чтобы увидеть, стоит ли развивать идею.',
        map: 'Я бы разложил каналы на три группы: быстрые тесты, платные размещения и форматы, которым нужен регулярный контент. Сразу станет видно, какой вариант реально запустить сейчас.',
        draft: 'Представьте первую проверку как короткий маршрут: кому показываем предложение, где размещаем сообщение, сколько длится тест и какой результат подтвердит интерес.'
      },
      sales: {
        recommendation: 'Я бы начал с одной понятной двери к покупке: места, где клиент замечает предложение, может задать вопрос и сразу понимает, как оплатить.',
        map: 'Нарисуйте для каждого канала маленькую сцену: первый контакт, диалог и действие, которое завершает покупку. Тогда варианты можно сравнить не по обещаниям, а по реальному пути клиента.',
        draft: 'Соберите короткий сценарий: человек замечает предложение, задаёт вопрос, получает ответ и оформляет покупку. После этого станет понятно, какой канал поддерживает весь маршрут.'
      },
      products: {
        ...
        map: 'Я бы разложил продукты по трём точкам: задача клиента, первый продукт и предложение после результата. На такой схеме сразу видны разрывы и повторы.',
        draft: 'Соберите маршрут из трёх элементов: входной продукт, основное решение и предложение после первого результата. Эту схему уже можно обсуждать с командой и дополнять.'
      }
    };
    ...
    return screen(`
      ...
      <div class="result-mascot">
        ...
        <p class="result-mascot-intro">
          ...
          <span>Аксолотль-Профессор и ещё один маскот Мирофактуры. Он смотрит на ответы как на систему и предлагает следующий шаг.</span>
        </p>
      </div>
      ...
      <div class="potap-figure">
        ...
        <p class="result-mascot-intro">
          <strong>Потап</strong>
          <span>Манул-Творец, креатор и визионер Мирофактуры. После вывода Аристарха он показывает, как может выглядеть решение.</span>
        </p>
      </div>
