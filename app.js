(function () {
  ...
  function potapAdvice(key) {
    ...
    const scenarios = {
      traffic: {
        recommendation: 'Начните с людей, которые уже пытаются решить ту же задачу другим способом: делают всё вручную, нанимают подрядчика или пользуются похожим сервисом. Спросите, что их не устраивает. Так вы увидите, кому ваш продукт действительно может пригодиться и чего эти люди от него ждут.',
        map: 'Не выбирайте аудиторию только потому, что она кажется большой или интересной. Посмотрите, у каких людей уже есть задача, которую решает ваш продукт. Поговорите с ними: они расскажут, что их не устраивает в нынешних способах и чего они ждут от нового решения.',
        draft: 'Не описывайте аудиторию только профессией, возрастом или размером бизнеса. Разделите людей по ситуациям: у кого-то выросла команда, кому-то перестал подходить старый инструмент, кто-то запускает новое направление. Для каждой ситуации напишите, какую задачу человек хочет решить и чем ему может помочь ваш продукт.'
      },
      sales: {
        recommendation: 'Если непонятно, с какого канала начать, попробуйте прямые продажи: переписку, созвон или личную встречу. Вы сразу услышите вопросы и сомнения клиентов. Повторяющиеся вопросы покажут, что нужно объяснить человеку перед покупкой и какой канал для этого подойдёт.',
        map: 'Смотрите не на популярность канала, а на то, как человеку удобнее принять решение о покупке. Если продукт важно увидеть в работе, нужен канал с демонстрацией. Если клиенту важно обсудить свою ситуацию, выбирайте канал, где можно поговорить с вами напрямую.',
        draft: 'У каждого канала должна быть своя роль: в одном человек знакомится с предложением, в другом узнаёт подробности, в третьем обсуждает свою задачу. Укажите, куда человек переходит после каждого канала. Так станет видно, какие каналы выполняют одну и ту же роль и на каком этапе человеку некуда перейти дальше.'
      },
      products: {
        recommendation: 'Начните с главного продукта — того, за которым к вам приходят в первую очередь. Посмотрите, какую часть задачи решает этот продукт и что клиенту после работы с вами всё равно приходится делать самому. То, с чем человек остаётся один, может стать хорошей идеей для следующего продукта.',
        map: 'Не выбирайте идею только потому, что она кажется яркой или необычной. Лучше оставьте идеи, которые помогают клиенту подготовиться к главному продукту или решить задачу, которая обычно возникает после него. Остальные можно предлагать отдельно, не пытаясь любой ценой включить их в линейку.',
        draft: 'Рядом с каждым продуктом напишите, кому он подходит, какую задачу решает и что человек получит. Затем проверьте, понятно ли клиенту, какой продукт выбрать сейчас и когда ему может понадобиться другой. Если это приходится долго объяснять, описания продуктов нужно сделать точнее.'
      }
    };
    ...
    return screen(`
      ...
      <div class="result-mascot">
        ...
        <p class="result-mascot-intro">
          ...
          <span>Аксолотль-Профессор, ещё один маскот Мирофактуры. Он сопоставляет ответы и определяет, с какой задачи лучше начать.</span>
        </p>
      </div>
