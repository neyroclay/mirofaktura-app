(function () {
  ...
  function quizMascotAdviceData() {
    ...
    const stepanObservations = {
      one: {
        word: 'У вас есть основной продукт, а рекомендации уже приводят клиентов. Проверьте, может ли новый человек без личного объяснения понять, какую задачу решает этот продукт и как его купить.',
        content: 'У вас есть основной продукт и контент, который приводит обращения. Проверьте, объясняет ли контент, кому подходит продукт и какое действие приводит человека к покупке.',
        ads: 'У вас есть основной продукт и реклама. До увеличения бюджета проверьте, совпадает ли обещание в рекламе с тем, что человек видит на странице продукта.',
        random: 'У вас есть основной продукт, но клиенты приходят из разных мест. Выберите один источник обращений и проверьте, можно ли несколько раз повторить этот результат.'
      },
      many: {
        word: 'Рекомендации приводят людей к вам, но несколько продуктов требуют особенно ясного выбора. Проверьте, понимает ли новый клиент, чем предложения отличаются и какой продукт подходит для его задачи.',
        content: 'Контент уже приводит обращения к нескольким продуктам. Проверьте, помогает ли он человеку сравнить предложения и выбрать продукт для своей задачи.',
        ads: 'Реклама приводит людей к нескольким предложениям. Проверьте, ведёт ли каждое объявление к одному понятному продукту, а не заставляет человека разбираться во всей линейке сразу.',
        random: 'У вас несколько продуктов, а обращения приходят из разных мест. Сначала покажите клиенту различия между предложениями и назовите ситуацию, для которой подходит каждый продукт.'
      },
      idea: {
        word: 'Сейчас у вас есть идея, а первые клиенты приходят по рекомендациям. Сформулируйте одно конкретное предложение, которое знакомый сможет точно пересказать другому человеку.',
        content: 'Контент уже собирает внимание, но идея ещё не стала предложением. Проверьте, может ли человек после публикации понять, что именно вы предлагаете и какой результат он получит.',
        ads: 'Если вы уже тестируете рекламу, сначала дайте ей конкретное предложение: один продукт, понятный результат и аудиторию, которой нужен этот результат.',
        random: 'Пока есть идея, а обращения появляются из разных источников. Сначала превратите идею в одно предложение, которое можно одинаково объяснить каждому новому клиенту.'
      }
    };

    const aristarkhConclusions = {
      traffic: 'Сейчас главная задача — не собрать длинный список площадок, а определить один канал для первого теста. Уточните, кому вы покажете предложение, сколько времени или денег выделите и по какому результату оцените канал.',
      sales: 'Вам нужна система выбора канала продаж. Сравните каналы по трём условиям: где находится ваша аудитория, какой формат общения ей подходит и какие ресурсы вы готовы вложить. После сравнения выберите один канал для проверки.',
      products: 'Вам нужно показать клиенту последовательность предложений: какой продукт он покупает первым, какой результат получает и какой продукт решает его следующую задачу. Сначала опишите эти три точки и только потом решайте, нужен ли новый продукт.'
    };

    const potapScenarios = {
      traffic: {
        recommendation: 'Представьте первый тест как небольшую экспедицию: одна аудитория, одно предложение, один канал и один измеримый результат. Материал поможет выбрать несколько кандидатов, а затем оставить один канал для проверки.',
        map: 'Разложите каналы как карту маршрутов: где можно быстро проверить интерес, где нужен бюджет, а где потребуется регулярный контент. Отметьте один маршрут, который вы проверите первым.',
        draft: 'Соберите паспорт первого теста: кому покажете предложение, где разместите сообщение, сколько дней продлится проверка и какой результат подтвердит интерес аудитории.'
      },
      sales: {
        recommendation: 'Представьте не витрину из десятков каналов, а одну дверь, через которую клиенту удобно перейти к покупке. Материал поможет найти эту дверь и подготовить первый тест.',
        map: 'Соберите карту продаж: для каждого канала отметьте, где происходит первый контакт, кто отвечает клиенту и какое действие завершает покупку. Сравнение покажет, какой канал проще проверить сейчас.',
        draft: 'Нарисуйте короткий сценарий продажи: человек замечает предложение, задаёт вопрос, получает ответ и оформляет покупку. Затем выберите канал, в котором этот сценарий можно проверить без лишних этапов.'
      },
      products: {
        recommendation: 'Представьте продукты как три остановки: знакомство, основная покупка и следующий результат. Материал поможет определить, какие остановки уже есть и какой переход пока нужно объяснить клиенту.',
        map: 'Разложите продукты на одной карте: с какой задачей приходит клиент, какой продукт решает эту задачу и какой продукт можно предложить после первого результата. На карте сразу станут видны разрывы и лишние повторения.',
        draft: 'Соберите первый маршрут клиента из трёх карточек: входной продукт, основное решение и предложение после первого результата. Этот черновик уже можно обсудить с командой или проверить на реальном клиентском сценарии.'
      }
    };

    return {
      stepan: stepanObservations[product]?.[channel] || stepanObservations.one.random,
      aristarkh: aristarkhConclusions[key] || aristarkhConclusions.traffic,
      potap: potapScenarios[key]?.[intent] || potapScenarios.traffic.recommendation
    };
  }
