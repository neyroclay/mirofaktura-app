(function () {
  ...
  const quiz = [
    ...
    },
    {
      ...
      note: 'Готово. Аристарх сопоставит ответы и предложит следующий шаг для выбранной задачи.',
      answers: [
        ['recommendation', 'Непонятно, с чего начать', 'Нужна короткая рекомендация и первый шаг.', 'Хорошо. Аристарх выделит задачу, с которой разумнее начать.'],
        ['map', 'Слишком много вариантов', 'Хочу сравнить варианты по понятным критериям.', 'Тогда сначала нужна общая карта, на которой видны различия между вариантами.'],
        ['draft', 'Не хватает понятной структуры', 'Нужны вопросы и последовательность, чтобы составить план под свою ситуацию.', 'Значит, нужен материал, который поможет последовательно составить свой план.']
      ]
    }
  ];
  ...
  function resultIntentCopy(key) {
    ...
    if (intent === 'draft') {
      return key === 'products'
        ? 'Ответьте на вопросы и соберите схему продуктовой линейки под свою ситуацию. Итог можно скопировать в заметки или рабочий чат.'
        : 'Ответьте на вопросы и составьте план первого теста под свою ситуацию. Итог можно скопировать в заметки или рабочий чат.';
    }
    return key === 'products'
  ...
  function quizMascotAdviceData() {
    ...
    const stepanObservations = {
      one: {
        word: 'Рекомендации уже приводят клиентов. Проверьте, понимает ли новый человек без личного объяснения, какую задачу решает продукт и как его купить.',
        content: 'Контент уже приводит обращения. Проверьте, объясняет ли он, кому подходит продукт и как перейти к покупке.',
        ...
        random: 'Клиенты приходят из разных мест. Выберите один источник обращений, результат которого можно проверить повторно.'
      },
      many: {
        word: 'Рекомендации приводят людей, но при нескольких продуктах выбор должен быть особенно ясным. Понимает ли новый клиент, чем предложения отличаются?',
        ...
        ads: 'Реклама ведёт к нескольким предложениям. Проверьте, приводит ли каждое объявление к одному понятному продукту.',
        random: 'Покажите клиенту различия между предложениями и назовите задачу, для которой подходит каждый продукт.'
      },
      idea: {
        word: 'Первые клиенты приходят по рекомендациям. Сформулируйте предложение, которое знакомый сможет точно пересказать другому человеку.',
        content: 'Контент уже собирает внимание, но идея ещё не стала предложением. Понимает ли человек после публикации, что именно вы предлагаете?',
        ads: 'Если вы тестируете рекламу, дайте ей конкретное предложение: один продукт, понятный результат и аудиторию, которой он нужен.',
        random: 'Превратите идею в одно предложение, которое можно одинаково объяснить каждому новому клиенту.'
      }
    };

    const aristarkhConclusions = {
      traffic: 'Выберите один канал для первого теста. Заранее определите аудиторию, бюджет или время и результат, по которому оцените канал.',
      sales: 'Сравните каналы по трём условиям: где находится аудитория, какой формат общения ей подходит и какие ресурсы вы готовы вложить. Затем выберите один канал для проверки.',
      products: 'Покажите клиенту три точки: первый продукт, результат после покупки и предложение для следующей задачи. Так станет видно, чего не хватает в линейке.'
    };

    const potapScenarios = {
      traffic: {
        recommendation: 'Соберите тест из четырёх частей: одна аудитория, одно предложение, один канал и один измеримый результат.',
        map: 'Разделите каналы на быстрые тесты, платные размещения и варианты, которым нужен регулярный контент. Затем выберите один канал.',
        draft: 'Запишите четыре пункта: кому показываете предложение, где размещаете сообщение, сколько длится проверка и какой результат подтвердит интерес.'
      },
      sales: {
        recommendation: 'Найдите одну «дверь» к покупке: канал, в котором клиенту проще заметить предложение, задать вопрос и оплатить.',
        map: 'Для каждого канала отметьте первый контакт, ответственного за диалог и действие, которое завершает покупку. Так варианты будет проще сравнить.',
        draft: 'Составьте короткий сценарий: человек замечает предложение, задаёт вопрос, получает ответ и оформляет покупку. Затем выберите канал для проверки.'
      },
      products: {
        recommendation: 'Представьте продукты как три остановки: знакомство, основная покупка и следующий результат. Посмотрите, какой переход пока не объяснён клиенту.',
        map: 'Разложите продукты по трём точкам: задача клиента, первый продукт и предложение после результата. На схеме станут видны разрывы и повторы.',
        draft: 'Соберите маршрут из трёх элементов: входной продукт, основное решение и предложение после первого результата. Эту схему можно обсудить с командой.'
      }
    };
